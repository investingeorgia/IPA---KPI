  ...
  if (!company) {
    return (
      <div>
        ...
        <Link to="/database/companies">← Back to companies</Link>
      </div>
    );
  }

  function handleSave() {
    updateCompany(id, { name, website, description });
    setSaved(true);
    setTimeout(() => setSaved(false), 1500);
  }

  const allLogs = getLogsForEntity('company', id);
  ...
  return (
    <div>
      <div style={{ fontSize: 13, color: 'var(--ink-3)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
        <Link to="/database/companies" style={{ color: 'var(--ink-3)', textDecoration: 'none' }}>{t('companies')}</Link>
        <span>›</span>
        <span style={{ color: 'var(--ink)' }}>{company.name}</span>
      </div>

      <h1 style={{ fontSize: 20, fontWeight: 700, margin: '0 0 20px' }}>{company.name}</h1>

      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 24, alignItems: 'start' }}>
        <Card>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: 4 }}>Name</label>
              <input className="input" value={name} onChange={e => setName(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: 4 }}>{t('website')}</label>
              <div style={{ display: 'flex', gap: 6 }}>
                <input className="input" style={{ flex: 1 }} value={website} onChange={e => setWebsite(e.target.value)} placeholder="domain.com" />
                {website && <a href={`https://${website}`} target="_blank" rel="noreferrer" style={{ color: 'var(--green)', alignSelf: 'center', fontSize: 13 }}>↗</a>}
              </div>
            </div>
            <div>
